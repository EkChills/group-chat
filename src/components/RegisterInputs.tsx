...
const RegisterInputs = () => {
  const {
    ...
    handleSubmit,
    formState: { errors },
  } = useForm<UserType>({ resolver: zodResolver(BasicSchemaWithPass) });
  ...
  const onSubmit: SubmitHandler<UserType> = async (data) => {
    ...
    try {
      ...
      toast({
        title:
          "Account registered successfully! We're thrilled to have you onboard.",
      });
      router.push("/login");
    } catch (error) {
      console.log(error);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col leading-[150%] text-center">
        ...
        <Link href={"/login"} className="text-[#633CFF] text-base font-medium">
          Login
        </Link>
      </div>
    </form>
